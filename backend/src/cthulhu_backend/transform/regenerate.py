...
def flow_disturb(
    ...
    """光流一致性破坏：按估计光流加权的平滑扰动场重采样后续帧。

    Farneback 光流在降采样亮度上估计，扰动幅度随局部运动增强（运动区域
    天然掩盖重采样痕迹），使帧间光流与真实运动解耦，破坏运动指纹。
    """
    ...
    smalls = [
        cv2.resize(luma(f).astype(np.float32), (sw, sh), interpolation=cv2.INTER_LINEAR)
        for f in work
    ]
    ...
    def one(index: int) -> np.ndarray:
        flow = cv2.calcOpticalFlowFarneback(
            smalls[index], smalls[index + 1], None, 0.5, 3, winsize, 3, 5, 1.2, 0
        )
        fx = cv2.resize(flow[..., 0], (w, h), interpolation=cv2.INTER_LINEAR)
        fy = cv2.resize(flow[..., 1], (w, h), interpolation=cv2.INTER_LINEAR)
        magnitude = np.sqrt(fx**2 + fy**2)
        weight = 0.3 + magnitude / (float(magnitude.max()) + 1e-6)
        field_x = rng.uniform(-1.0, 1.0, (6, 8)).astype(np.float32)
        field_y = rng.uniform(-1.0, 1.0, (6, 8)).astype(np.float32)
        dx = cv2.resize(field_x, (w, h), interpolation=cv2.INTER_LINEAR) * strength * weight
        dy = cv2.resize(field_y, (w, h), interpolation=cv2.INTER_LINEAR) * strength * weight
        frame = work[index + 1]
        # uint8 输入必须用原始帧做 remap：work 已是 [0,1] float，
        # 误用它会把首帧（0~255）与后续帧（0~1）混进同一数组。
        if is_u8:
            base = frames[index + 1]
        else:
            base = (np.clip(frame, 0, 1) * 255).round().astype(np.uint8)
        ...
        if is_u8:
            return warped
        return warped.astype(original_dtype) / 255.0

    out = [frames[0] if is_u8 else work[0]]
    # Farneback 为 C++ 重计算且会释放 GIL，顺序执行避免 OpenCV 线程析构崩溃。
    out.extend(one(index) for index in range(len(work) - 1))
    return np.stack(out)
...
def texture_inject(
    ...
    """纹理重注入：向低纹理区域注入合成高频纹理，改变纹理/边缘描述子。"""
    if strength <= 0:
        return frames
    import cv2
    ...
    def one(frame: np.ndarray) -> np.ndarray:
        ...
        energy = np.abs(band)
        threshold = float(np.percentile(energy, 70))
        mask = np.clip(1.0 - energy / max(threshold, 1e-6), 0.0, 1.0)
        mask = blur(mask, 2.0)
        noise = rng.standard_normal(luma.shape).astype(np.float32)
        grain = noise - blur(noise, 1.0)
    ...
    attacked = np.stack([one(work[i]) for i in range(len(work))])
    return _as_original(attacked, original_dtype)


def multiscale_perturb(
    frames: np.ndarray,
    strength: float,
    rng: np.random.Generator,
    levels: int = 4,
) -> np.ndarray:
    """DMFF 多尺度特征扰动：对拉普拉斯金字塔各尺度叠加平滑随机扰动。

    多尺度检测器在任意分辨率/尺度上做特征提取，单尺度扰动只在固定粒度
    生效；逐级衰减的带限扰动让 8/16/32 各尺度签名同步偏移，观感仍平滑。
    """
    if strength <= 0:
        return frames
    import cv2
    ...
    def one(frame: np.ndarray) -> np.ndarray:
        ...
        if frame.ndim == 3:
            ...
            luma = frame
            color = False
        perturb = np.zeros_like(luma)
        sigma = 1.0
        amplitude = strength
        for _ in range(levels):
            field = blur(rng.standard_normal(luma.shape).astype(np.float32), sigma)
            scale = float(field.std())
            perturb = perturb + field * (amplitude / max(scale, 1e-6))
            sigma *= 2.0
            amplitude *= 0.55
        result = np.clip(frame + perturb[..., None] if color else frame + perturb, 0.0, 1.0)
...
def complexity_trap(
    ...
    """VCGDD 复杂度陷阱：向低复杂度区域注入纹理，把局部复杂度拉向中位数。

    复杂度类检测器靠"异常平坦/异常纹理"区域定位隐写痕迹；把复杂度分布
    压平后，全图不再有可被指认的低复杂度区，同时高纹理区观感不受影响。
    """
    if strength <= 0:
        return frames
    import cv2
    ...
    def one(frame: np.ndarray) -> np.ndarray:
        ...
        if frame.ndim == 3:
            ...
            color = False
        band = blur(luma, 1.0) - blur(luma, 3.0)
        energy = blur(np.abs(band), 4.0)
        target = float(np.median(energy))
        deficit = np.clip(target - energy, 0.0, None) / max(target, 1e-6)
        grain = rng.standard_normal(luma.shape).astype(np.float32)
        grain = blur(grain, 0.6) - blur(grain, 1.4)
        grain /= max(float(grain.std()), 1e-6)
        injected = strength * deficit * grain
        result = np.clip(frame + injected[..., None] if color else frame + injected, 0.0, 1.0)
        return result.astype(np.float32)
...
def temporal_blur(frames: np.ndarray, strength: float) -> np.ndarray:
    """时序模糊：帧间时域平滑，把每帧向相邻帧均值混合。

    破坏逐帧精确匹配与帧差结构，视觉上呈轻微运动模糊；首尾帧保持不变，
    强度 0.5 即完全与邻帧均值混合。确定性原语，不需要随机源。
    """
    ...
    if original_dtype == np.uint8:
        work /= 255.0
    out = work.copy()
    out[1:-1] = (1.0 - strength) * work[1:-1] + strength * 0.5 * (work[:-2] + work[2:])
    if original_dtype == np.uint8:
