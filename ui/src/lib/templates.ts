...
export function payloadOf(template: TemplateInfo): TemplatePayload {
  ...
  return {
    ...
    fftMag: (raw.fftMag as number) ?? 0,
    dwtDetail: (raw.dwtDetail as number) ?? 0,
    hsvJitter: (raw.hsvJitter as number) ?? 0,
    nonintRatio: (raw.nonintRatio as number) ?? 0,
    jitter: (raw.jitter as number) ?? 0,
    perspective: (raw.perspective as number) ?? 0,
    warp: (raw.warp as number) ?? 0,
    flowDisturb: (raw.flowDisturb as number) ?? 0,
    textureInject: (raw.textureInject as number) ?? 0,
    multiscale: (raw.multiscale as number) ?? 0,
    complexityTrap: (raw.complexityTrap as number) ?? 0,
    facePerturb: (raw.facePerturb as number) ?? 0,
    temporalBlur: (raw.temporalBlur as number) ?? 0,
