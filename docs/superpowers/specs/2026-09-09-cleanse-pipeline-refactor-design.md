...
- **音画同步与分段取整（2026-09-10 修复）**：验收标准是「成片音画不能不同步」，
  ...
  **50 个镜头会累积 14 帧（467ms）**；而音轨是按整段均匀拉伸的，多出来的
  ...
  即放慢后的音轨本身不会引入累积错位。
- **重启语义（2026-09-14 修正）**：`jobs.restore()` 不再把未完成任务整批
  标记为 failed。引擎重启（用户退出 / 引擎崩溃 / 看门狗强制重启）后：排队中
  的任务直接回到队列；运行中的任务整条重跑（管线没有分段断点），并累计
  `resume_count`，超过 `MAX_AUTO_RESUME`（3 次）仍被中断才标记 failed 并提示
  手动重试——既保留 0.6.0 之前「重启后自动续跑」的连续性，又不会让坏素材把
  引擎拖进「崩溃 → 重启 → 重跑」死循环。子任务开始/结束时即时落库，所以重启
  只重跑真正被打断的那几条，已完成项不再重跑。暂停中的任务重启后保持暂停，
  中断的子任务退回队列，避免「继续」只改状态却没有任务在跑。
- **守护策略（2026-09-14 修正，0.6.3）**：现场反馈「大量任务以进程重启中断
  失败」，根因是看门狗把「忙」当「死」——原先 `/api/health` 连续 3 次 5 秒
  超时（约 45 秒）就 SIGTERM→SIGKILL 强杀引擎，而清洗一条视频要几十分钟，
  16GB 机器在内存压力下事件循环本来就可能几十秒答不上探针，于是变成「杀掉
  在跑的任务 → 重启从 0 重跑 → 再被杀」的自我维持循环。现在：
  1. 后端用独立线程写心跳（`heartbeat.py`，`CTHULHU_ENGINE_HEARTBEAT` 配置；
     与事件循环解耦，numpy/torch 释放 GIL 时照样刷新），内容是「在跑任务数 +
     最近进展时间」；
  2. 主进程判定强杀要同时满足「探针连续失败」+「没有任务在推进」+「持续
     `STALL_GRACE_MS`（3 分钟）」，任一条件不成立就继续等（`supervisor.mjs`）；
  3. 引擎每次退出或被强杀都往崩溃日志追加一行带时间戳的原因，并把原因经
     `CTHULHU_RESTART_CAUSE` 传给下一次启动，写进续跑任务的说明；
  4. 同一任务被打断 ≥`SAFE_MODE_AFTER`（2）次即进入安全模式：本次重启并发降为
     1，用吞吐换跑完；
  5. 内存预算按机器档位分（<4GB 40%、4~24GB 45%、≥24GB 55%），并给 torch 按
     并发分线程（10 核跑 2 条任务是 5+5，而不是 10+10 互抢）。
- **推理设备降级（2026-09-14，0.6.3）**：上述「引擎反复重启」的真凶是
  **MPS 原生崩溃**——复现（冻结包、2 条 720p 并发清洗，约 40 秒后）拿到
  `exit code=-6`（SIGABRT）与 stderr 的
  `objc: Cannot form weak reference to instance of class MPSGraph`，即 PyTorch
  MPS 后端在 dealloc 时的已知缺陷，进程无法捕获、必崩。实测吞吐：MPS
  38ms/帧 vs CPU 139ms/帧（720p，32 帧均值），所以不能简单退回 CPU。策略：
  - 设备偏好收敛到 `purify_device`（`auto` 默认：有 GPU 用 GPU；`cpu`/`mps`/
    `cuda` 可显式指定，环境变量 `CTHULHU_PURIFY_DEVICE` 优先，指定但不可用
    的设备退回 CPU）；
  - 守护进程检测到**异常退出**（信号或非零退出码）时置 `CTHULHU_RESTART_ABNORMAL=1`，
    后端在 `restore()` 里把设备降级为 CPU 并落盘设置，重启后的续跑不再掉进
    同一个坑；用户主动退出（正常退出）不触发降级。
