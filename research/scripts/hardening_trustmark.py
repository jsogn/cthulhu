#!/usr/bin/env python3
"""TrustMark 加固实验：嵌入强度 + 多副本冗余（研究用途）。

在扩散净化攻击（sd-turbo）下比较：
- 提高 TrustMark 嵌入强度 WM_STRENGTH（1.0 / 1.5 / 2.0 / 3.0）
...
def decoder_probs(tm: TrustMark, img: Image.Image) -> np.ndarray:
    stego = img.convert("RGB").resize(
        (tm.model_resolution_dec, tm.model_resolution_dec), Image.BILINEAR
    )
    tensor = transforms.ToTensor()(stego).unsqueeze(0).to(tm.device) * 2.0 - 1.0
    with torch.no_grad():
...
def main() -> None:
    ...
    parser.add_argument("--attack-strength", type=float, default=0.30)
    parser.add_argument(
        "--out", type=Path, default=ROOT / "research" / "output" / "hardening_trustmark"
    )
    ...
    payload = random_bits(rng, 100)
    target = np.array([int(x) for x in payload], dtype=np.float32)
    ...
    for strength in (1.0, 1.5, 2.0, 3.0):
        ...
        for image_id in range(args.images):
            ...
            watermarked = tm.encode(
                cover, payload, MODE="binary", WM_STRENGTH=strength
            )
            ...
            prob = decoder_probs(tm, attacked)
            ...
            rows.append(
                {
                    ...
                    "image_id": image_id,
                    "bit_accuracy": float(((prob > 0.5) == target).mean()),
        ...
        avg_ba = float(((avg_prob > 0.5) == target).mean())
        subset = [r for r in rows if r["wm_strength"] == strength]
        for row in subset:
            row["multi_copy_ba"] = avg_ba
        print(
            f"WM_STRENGTH={strength:.1f} single_BA="
    ...
    with md_path.open("w") as handle:
        handle.write("# TrustMark 加固实验：嵌入强度 vs 多副本冗余\n\n")
        handle.write(
            f"- 攻击：sd-turbo img2img strength={args.attack_strength}，"
            f"{args.steps} steps\n"
        )
        handle.write(f"- 样本：{args.images} 张合成图；消息：固定 100-bit\n\n")
        handle.write(
            "| WM_STRENGTH | single BA | multi-copy BA | PSNR vs 原图 | SSIM vs 原图 |\n"
        )
        handle.write("| ---: | ---: | ---: | ---: | ---: |\n")
        for strength in (1.0, 1.5, 2.0, 3.0):
            subset = [r for r in rows if r["wm_strength"] == strength]
